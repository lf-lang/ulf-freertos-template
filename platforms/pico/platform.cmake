...
#
# Required environment variables/CMake variables: - PICO_SDK_PATH: Path to the
# Pico SDK - FREERTOS_KERNEL_PATH (optional): Path to FreeRTOS kernel with
# RP2040 port (auto-detected from submodule if not set) - REACTOR_UC_PATH: Path
# to reactor-uc repository
#
# This file: 1. Locates and initializes the Pico SDK 2. Locates and imports the
# FreeRTOS kernel for RP2040 3. Defines platform_configure_target() function for
  ...
  if(DEFINED ENV{FREERTOS_KERNEL_PATH})
    ...
    if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/FreeRTOS/FreeRTOS/Source")
      set(FREERTOS_KERNEL_PATH
          "${CMAKE_CURRENT_SOURCE_DIR}/FreeRTOS/FreeRTOS/Source")
      message(
        STATUS
          "Found FreeRTOS kernel in project submodule: ${FREERTOS_KERNEL_PATH}")
    else()
      message(
        FATAL_ERROR
...
set(FREERTOS_CONFIG_PATH
    ...
    CACHE PATH "FreeRTOS config path for Pico")
...
function(platform_configure_target TARGET_NAME)
  ...
  # Add platform-specific include directory for hooks.h, FreeRTOSConfig.h, and
  # other headers
  ...
  target_link_libraries(${TARGET_NAME} PUBLIC pico_stdlib pico_sync)
  ...
  target_compile_definitions(${TARGET_NAME} PRIVATE PICO_PLATFORM=1)
